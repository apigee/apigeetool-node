...
module.exports.run = function(opts, cb) {
  ...
  if (!deploymentVersionProvided(opts)) {
    ...
    // update the revision if provided with opt.deploymentVersion
    // Run each function in series, and collect an array of results.
    series.push(
      function (done) {
        updateApiProxyRevision(opts, request, done);
      },
      function (done) {
        runNpm(opts, request, done);
      }
...
function updateApiProxyRevision(opts, request, done) {
  if (!opts.revision) {
    done(new Error("--revision needs to be supplied for update proxy revision request"));
    return;
  }

  var pd = path.join(opts.directory, ProxyBase);
  var uri = util.format('%s/v1/organizations/%s/apis/%s/revisions/%s',
                    opts.baseuri, opts.organization, opts.api, opts.revision);
  if (opts.debug) {
    console.log('Calling %s', uri);
  }
  if (opts.verbose) {
    console.log('Updating revision %d of API %s', opts.revision,
               opts.api);
  }
  // The only way to do this is to import a ZIP. What fun.
  ziputils.zipDirectory(pd, ProxyBase, function(err, zipBuf) {
    if (err) {
