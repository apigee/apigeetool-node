...
module.exports.getPathInfo = function(deployments, opts, cb) {
  ...
  async.eachLimit(deployments, opts.asyncLimit, function(item, done) {
    ...
    if (opts.debug) {
      console.log('Final result: err = %j result = %j', err, deployments);
    }
    cb(err);
  });
};
  ...
  request.get(uri, function(err, req, body) {
        if (err) {
          ...
          if (req.statusCode === 200) {
            ...
            done(new Error(util.format('HTTP Error getting proxy: %d', req.statusCode)));
          }
        }
      }
  );
    ...
    request.get(uri, function(err, req, body) {
        if (err) {
          ...
          if (req.statusCode === 200) {
            ...
            done(new Error(util.format('HTTP Error getting virtual host: %d', req.statusCode)));
          }
        }
      }
    );
...
function finishProxyUri(deployment, protocol, port, proxyBasePath, hostAlias) {
  ...
  uri = url.resolve(uri, proxyBasePath);
