...
module.exports.run = function (opts, cb) {
  ...
  if (!deploymentVersionProvided(opts)) {
    series.push(
      function (done) {
    ...
    series.push(
      function (done) {
        updateSharedFlowVersion(opts, request, done);
      }
    )
  }
...
function updateSharedFlowVersion(opts, request, done) {
  if (!opts.revision) {
    done(new Error("--revision needs to be supplied for update SharedFlow revision request"));
    return;
  }

  var pd = path.join(opts.directory, SharedFlowBase);
  var uri = util.format('%s/v1/organizations/%s/sharedflows/%s/revisions/%s',
    opts.baseuri, opts.organization, opts.name, opts.revision);
  if (opts.debug) {
    console.log('Calling %s', uri);
  }
  if (opts.verbose) {
    console.log('Updating revision %d of SharedFlow %s', opts.revision,
      opts.name);
  }
  // The only way to do this is to import a ZIP. What fun.
  ziputils.zipDirectory(pd, SharedFlowBase, function(err, zipBuf) {
    if (err) {
...
function deploymentVersionProvided(opts) {
  if (Number.isInteger(opts.revision)) {
    return true;
  }

  var revisionNumberProvided = false;
  try {
    opts.revision = parseInt(opts.revision);
    revisionNumberProvided = Number.isInteger(opts.revision);
  } catch (e) {
    console.error("--revision is not a parsable integer");
  }

  return revisionNumberProvided;
}
...
function getDeploymentInfo(opts, request, done) {
  ...
  try {
    ...
    done(new Error(util.format('SharedFlow base directory %s does not exist',
      opts.directory)));
    return;
  }
  if (!ds.isDirectory()) {
    done(new Error(util.format('SharedFlow base directory %s is not a directory',
      opts.directory)));
    return;
  }
  ...
  request.get(util.format('%s/v1/o/%s/sharedflows/%s',
    ...
    function (err, req, body) {
      if (err) {
        ...
        if (opts.verbose) {
          console.log('SharedFlow %s does not exist. Going to create revision 1',
            opts.name);
        }
        done();
        ...
        if (opts.verbose) {
          console.log('Going to create revision %d of SharedFlow %s',
...
function createSharedFlowBundle(opts, request, done) {
  ...
  if (opts.verbose) {
    console.log('Creating revision %d of SharedFlow %s', opts.deploymentVersion,
      opts.name);
  }
  // The only way to do this is to import a ZIP. What fun.
  var zipBuf = ziputils.makeOneFileZip(SharedFlowBase, rootEntryName, rootDoc);
  // For debugging
  //fs.writeFileSync('./test.zip', zipBuf);
  request({
    uri: uri,
...
function sharedFlowCreationDone(err, req, body, opts, done) {
  if (err) {
    ...
    done(new Error(util.format('SharedFlowBundle creation failed. Status code %d',
      req.statusCode)));
  }
}
  ...
  async.eachLimit(fileNames, opts.asynclimit, function (fileName, itemDone) {
    ...
    var postReq = request({
      ...
      if (err) {
        ...
        itemDone(new Error(util.format('Error uploading sharedflow: %s',
          req.statusCode)));
      }
    });
