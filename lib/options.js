...
function checkPropertySync(opts, descriptor, propName) {
  var desc = descriptor[propName];
  if (desc === null || desc === undefined) {
    throw new Error(util.format('Invalid property %s', propName));
  }
  if (desc.required && !opts[propName]) {
      throw new Error(util.format('Missing required option "%s"', propName));
  } else {
    if (opts[propName] && (desc.secure === true)) {
      makeSecure(opts, propName);
    }
...
module.exports.getHelp = function(descriptor) {
  ...
  _.each(_.pairs(descriptor), function(d) {
    tab.push([d[0],
