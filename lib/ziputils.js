...
module.exports.enumerateNodeDirectory = function(baseDir, remoteNpm, cb) {
  try {
    var fileList = [];
    var packageFile = path.resolve(baseDir, 'package.json');
    var packageObj = JSON.parse(fs.readFileSync(packageFile, 'utf8'));
    var devDependencies = Object.keys(packageObj.devDependencies || {});

    visitDirectory(baseDir, fileList, '', '', 'node', true, remoteNpm, devDependencies);
...
function visitDirectory(dn, fileList, resourceNamePrefix,
                        zipEntryPrefix, resourceType, isNode, remoteNpm, devDependencies) {
      ...
      if (isNode && (file === 'node_modules')) {
        if (remoteNpm !== true) {
          ...
          visitDirectory(fullName, fileList, 'node_modules_',
                        'node_modules/', resourceType, false, remoteNpm, devDependencies);
        }
        ...
        if (resourceNamePrefix !== 'node_modules_') {
          ...
          visitDirectory(fullName, fileList, prefix, '', resourceType, false, remoteNpm, devDependencies);
        } else if (devDependencies && devDependencies.indexOf(file) === -1) {
          addResource({
              fileName: fullName,
              resourceType: resourceType,
